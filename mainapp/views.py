...
def get_links_menu_category():
    if settings.LOW_CACHE:
        key = 'links_menu'
        links_menu_category = cache.get(key)
        if links_menu_category is None:
            links_menu_category = ProductCategory.objects.filter(is_active=True)
            cache.set(key, links_menu_category)
...
def products(request, pk=None, page=1):
    title = 'продукты'
    links_menu_category = get_links_menu_category()
